...
struct ReflectionPad2dBackwardDetKernelFunctor {
  void operator()(sycl::nd_item<3> item) const {
    const int64_t tid = static_cast<int64_t>(item.get_global_id(2));
    const int64_t stride = static_cast<int64_t>(item.get_global_range(2));

    const int64_t width = input_dim_x_ + pad_l_ + pad_r_;
    const int64_t height = input_dim_y_ + pad_t_ + pad_b_;
    const int64_t N = height * width;

    const int64_t end = nbatch_ * nplane_ * input_dim_y_ * input_dim_x_;

    for (int64_t input_xy = tid; input_xy < end; input_xy += stride) {
      // linear index over B*C*H*W (contiguous)
      const int64_t pos_xy = input_xy % (input_dim_x_ * input_dim_y_);
      const int64_t inp_row = pos_xy / input_dim_x_;
      const int64_t inp_col = pos_xy % input_dim_x_;

      const bool is_top = (inp_row >= 1) && (inp_row <= pad_t_);
      const bool is_bottom =
          (inp_row < input_dim_y_ - 1) && (inp_row >= input_dim_y_ - pad_b_ - 1);
      const bool is_left = (inp_col >= 1) && (inp_col <= pad_l_);
      const bool is_right =
          (inp_col < input_dim_x_ - 1) && (inp_col >= input_dim_x_ - pad_r_ - 1);

      const int64_t bottom_row = input_dim_y_ - 1;
      const int64_t dist_from_bottom = std::abs(inp_row - bottom_row);
      const int64_t dist_from_right = std::abs(inp_col - (input_dim_x_ - 1));
      ...
      const int64_t batch_idx = input_xy / (nplane_ * input_dim_x_ * input_dim_y_);
      const int64_t channel_idx = (input_xy / (input_dim_x_ * input_dim_y_)) % nplane_;
      const int64_t grad_output_base_offset = batch_idx * (nplane_ * N) + channel_idx * N;
      ...
      if (is_top) {
        // Reflect across the top border: row mirrors above pad_t_,
        // column stays at its padded position.
        accum_grad_at(pad_t_ - inp_row, pad_l_ + inp_col);

        if (is_left) { // top-left corner
          // Row mirrors above pad_t_, column mirrors left of pad_l_.
          accum_grad_at(pad_t_ - inp_row, pad_l_ - inp_col);
        } else if (is_right) { // top-right corner
          // Row mirrors above pad_t_, column mirrors right of
          // the right border at pad_l_ + input_dim_x_ - 1,
          // offset by the distance from inp_col to the right edge.
          accum_grad_at(
              pad_t_ - inp_row,
              pad_l_ + input_dim_x_ - 1 + dist_from_right);
        }
      }

      if (is_bottom) {
        // Reflect across the bottom border: row mirrors below
        // pad_t_ + bottom_row, column stays at its padded position.
        accum_grad_at(pad_t_ + bottom_row + dist_from_bottom, pad_l_ + inp_col);

        if (is_left) { // bottom-left corner
          // Row mirrors below pad_t_ + bottom_row, column mirrors
          // left of pad_l_, offset by inp_col.
          accum_grad_at(pad_t_ + bottom_row + dist_from_bottom, pad_l_ - inp_col);
        } else if (is_right) { // bottom-right corner
          // Row mirrors below pad_t_ + bottom_row, column mirrors right
          // of the right border at pad_l_ + input_dim_x_ - 1,
          // each offset by their respective distances from the corner.
          accum_grad_at(
              pad_t_ + bottom_row + dist_from_bottom,
              pad_l_ + input_dim_x_ - 1 + dist_from_right);
        }
      }
      ...
      if (is_left) {
        ...
        // position, column mirrors left of pad_l_.
        accum_grad_at(inp_row + pad_t_, pad_l_ - inp_col);
      }

      if (is_right) {
        // Reflect across the right border: row stays at its padded
        // position, column mirrors right of pad_l_ + input_dim_x_ - 1,
        // offset by the distance from inp_col to the right edge.
        accum_grad_at(inp_row + pad_t_, pad_l_ + input_dim_x_ - 1 + dist_from_right);
      }

      // Center (always): direct padded position, no reflection.
      accum_grad_at(inp_row + pad_t_, inp_col + pad_l_);

      grad_input_[input_xy] = partial;
    }
  }
...
void reflection_pad2d_backward_det_template(
  ...
  const int64_t input_dim_xy = input_dim_x * input_dim_y;
  const int64_t total_elements = nbatch * nplane * input_dim_xy;
  ...
  // Use a flat 1D range (not 3D nbatch x nplane x ...) because the kernel
  // iterates over all input elements with a grid-stride loop to avoid atomics.
  sycl_kernel_submit(
      sycl::range<3>(1, 1, work_group_size * work_group_num),
      sycl::range<3>(1, 1, work_group_size),
      queue,
      kfn);
}
